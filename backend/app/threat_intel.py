...
def enrich_vulnerability(cve: str | None, severity: str, title: str, description: str | None = None) -> dict[str, object]:
    ...
    epss_lookup = get_epss_score(cve_key) if cve_key else None

    if cve_key:
        epss = epss_lookup if epss_lookup is not None else 0.0
        ...
        cvss_source = "severity_derived"
        epss_source = "epss" if epss_lookup is not None else "unavailable"
        evidence_mode = "cve_enriched"
    else:
        epss = 0.0
        exploit_maturity = "not_applicable"
        threat_context = "observed_exposure"
        cvss = 0.0
        cvss_source = "not_applicable"
        epss_source = "not_applicable"
